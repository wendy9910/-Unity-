...
public class hairmesh : MonoBehaviour
{
    ...
    void MeshGenerate()
    {
        ...
        mesh.name = "Hair Grid";
        int[] verticeBox = new int[HairN];
        int[] triangleBox = new int[HairN];

        verticeBox[Num] = Mathf.Abs(MousePointPos.Count-VoldPos);
        

        Vector3[] vertice = new Vector3[verticeBox[Num]];
        Vector2[] uv = new Vector2[verticeBox[Num]];//texture
        Vector4[] tangents = new Vector4[verticeBox[Num]];
        Vector4 tangent = new Vector4(1f, 0f, 0f, -1f);

        for (int i = VoldPos,j=VoldPos; i < verticeBox[Num]; i++,j++)//Vector3轉Vector2
        {
        ...
        int t = ToldPos;//初始三角形
        point = (((verticeBox[Num] / (3 + (width - 1) * 2) - 1)) * 2 * width)*6;//計算網格數 * 6 total三角形Count
        triangleBox[Num] = Mathf.Abs(point - ToldPos);

        int[] triangles = new int[point];//計算需要多少三角形點座標
                
        int k = 0;//累加
        for (int vi = 0, x = 1; x <= triangleBox[Num]/6; x++, vi += k)
        {
            t = SetQuad(triangles, t, vi, vi + 1, vi + 3 + (2 * (width - 1)), vi + 4 + (2 * (width - 1)));
            if (x % (width * 2) != (triangleBox[Num]/6) % (width * 2)) k = 1;
            else k = 2;
        }
